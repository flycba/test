...
with tf.Session() as sess:
    ...
    for epoch in range(training_epochs):
        ...
        if (epoch + 1) % display_step == 0:
            ...
            print("Epoch:", '%04d' % (epoch + 1), "cost=", "{:.9f}".format(c), "CX=", sess.run(cx), "CY=", sess.run(cy),
                  "Mean=", "{:.9f}".format(m))

    print("Optimization Finished!")
    training_cost = sess.run(cost, feed_dict={X: train_X, Y: train_Y})
    print("Training cost=", training_cost, "CX=", round(sess.run(cx), 3), "CY=", round(sess.run(cy), 2), "R=",
          round(m, 2), '\n')
